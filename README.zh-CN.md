...
未配置认证密钥且节点尚未加入网络时，通过懒猫应用域名打开 Web 界面，点击登录会跳转到 Tailscale 或 Headscale 的认证页面。认证完成后，节点状态会保存在应用数据目录中，重启应用不需要重新登录。

如果安装时填写了认证密钥，应用会自动加入网络，不会再显示首次登录流程。此时 Web 界面的“管理”入口可能打开 `http://<Tailscale-IP>:5252/?check=now`。这是 Tailscale 自带的 Tailnet 内管理地址，只能从已经加入同一 Tailnet 的设备访问，不是公网地址，也不是 `host.lzcapp` 地址。
